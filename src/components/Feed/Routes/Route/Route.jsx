...
const Route = () => {  
    ...
    // }
    const [currentValue, setCurrentValue] = useState('.  Califica la ruta para ver la media')
    ...
    async function rating(value,routeIdL){
        if(elementId=== routeIdL){
            ...
            await axios.post(`http://localhost:8080/ratings/${routeIdL}`,{rating:value},{headers: {authorization: user?.token,}});
          ...
          <div id="mobileFirst" className="container px-4 px-lg-5 mt-5">
            <div  className="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center">
              ...
              <div id={elements._id} onClick={getElementOnClick}>
                <Rate allowHalf defaultValue={2.5}  onFocus={(value) => {showValue(value)}} onChange={(value) => {rating(value, elements._id)}} value={rates}/>
              </div>
              
                </div>
                <p id={elements._id+"p"} onClick={getElementOnClick}> Puntuación media de la ruta {currentValue}</p>
              </div>
            </div>
          </div>
        </div>
